...
```r
 weekly_counts_by_site_birth_month <- 
    count_by_period(
       tbl = your_table,
       date_col = "d_471593703",
       period = "week",
     ...
     rename(site = d_827220437, birth_month = d_564964481) %>%
     show_query() %>%
     collect() %>%
     print()
```
...
FROM (
  SELECT
    ...
    `d_827220437`,
    `d_564964481`,
    TIMESTAMP_TRUNC(CAST(`d_471593703` AS TIMESTAMP), WEEK(SUNDAY)) AS `period_start`
  FROM `FlatConnect.participants_JP`
  WHERE (NOT((`d_471593703` IS NULL)))
) `q01`
GROUP BY `period_start`, `d_827220437`, `d_564964481`
ORDER BY `period_start`
Job complete
Billed: 0 B
...
# A tibble: 747 × 4
   ...
   <dttm>              <chr>     <chr>             <int>
 1 2022-05-29 00:00:00 531629870 03                    1
 2 2022-05-29 00:00:00 303349821 05                    1
 3 2022-05-29 00:00:00 303349821 01                    2
 4 2022-05-29 00:00:00 531629870 11                    1
 5 2022-05-29 00:00:00 303349821 08                    1
 6 2022-05-29 00:00:00 531629870 04                    1
 7 2022-05-29 00:00:00 531629870 10                    1
 8 2022-06-05 00:00:00 657167265 NA                    1
 9 2022-06-05 00:00:00 657167265 01                    2
10 2022-06-05 00:00:00 548392715 07                    1
# ℹ 737 more rows
# ℹ Use `print(n = ...)` to see more rows
```
